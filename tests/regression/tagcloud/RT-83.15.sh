...
# ABOUTME: RT-83.15 - float lead terms move ahead while only configured ranks float.
...
#   Float lead terms move ahead while only configured ranks float.

# shellcheck source=_helpers.sh
source "$(dirname "${BASH_SOURCE[0]}")/_helpers.sh"

run_test() {
    run_tagcloud_case "83.15"
}
